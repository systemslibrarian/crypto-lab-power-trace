 ...
 *    an unknown backdrop, or a `color-mix()`. This page is full of them, and
 *    they are exactly the surfaces that carry its verdicts. Every `.panel` —
 *    which is every section on this page — is a
 *    `linear-gradient(165deg, var(--panel), var(--panel-2))`, so NOTHING here
 *    sits on a flat colour. On top of that, `.indicator.alarm` and
 *    `.indicator.held` (the two security verdicts) paint
 *    `color-mix(in srgb, var(--danger|--warn) 12%, var(--panel))`,
 *    `.byte-box.leaked` and `.bit-cell.one` are 16% mixes over
 *    `--chart-surface`, `.status-region` is a 5% accent mix, `code` and
 *    `thead th` are 14% mixes, and the hero aside is a
 *    `color-mix(in oklab, var(--accent) 8%, transparent)`. axe files every one
 *    of them under `incomplete`, so a violations-only gate measured the contrast
 *    of almost nothing this lab actually says.
 ...
 *    `button:disabled { opacity: 0.55 }`, and this page ships no disabled
 *    button at all — which is exactly why the walk is the check rather than a
 *    reading of the stylesheet.
 *
 ...
 *    boxes are the `.tablewrap` scrollers — the `overflow-x: auto` wrappers
 *    around the data tables that are the accessible alternative to every canvas
 *    figure, and which only exist once an exhibit has been run — plus
 *    `.sr-only`. Content scrolled past those is not dimmed or partly drawn — it is absent from the
 *    frame, and asking what colour it sits on has no answer. Its rect is still
 ...
 *    on the content that IS in view, which is measured for real.
 *
 ...
 *    draws every figure into a `<canvas>` rather than SVG, so the sibling walk
 *    finds nothing to do. Its only SVG is the shared top bar's two
 *    `aria-hidden` glyphs. Canvas pixels are outside BOTH oracles, which is why
 *    each chart also ships a `<details class="data">` table alternative that IS
 *    walked — see the header of `gate.ts`. The branch is kept because the alternative is a
 *    helper that silently mismeasures the first labelled diagram anyone adds: an
 ...
 *
 * FIRST, GENERATED CONTENT: the walk iterates ELEMENTS, and a `::before` /
 * `::after` is not one, nor is it one to axe's `color-contrast` rule. That gap
 * is closed rather than noted — `nontext.ts` measures every pseudo-element on
 * the page, both the ones that carry a glyph and the ones that are only a drawn
 * shape, against its host's composited backdrop, and the gate asserts on the
 * result.
 *
 * SECOND, CANVAS PIXELS. Seven `<canvas>` figures carry this lab's charts, and
 * nothing here or in axe can read a pixel one of them painted. That is not
 * waved away: every chart is drawn by `src/ui/charts.ts` from the same theme
 * tokens this walk measures everywhere else (`--danger-line`, `--chart-axis`,
 * `--chart-grid`, `--text` on `--chart-surface`), each canvas carries
 * `role="img"` with a written label that states the result in words, and each
 * has a `<details class="data">` table alternative whose every cell IS walked.
 * The oracle therefore covers the text a reader relies on; the chart is a
 * second copy of it.
 */
...
/** Measure every element on the page that owns a text node of its own. */
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * page's `body`: a `linear-gradient(165deg, var(--bg0), var(--bg1))` runs
     * the full height of a document several screens tall, so text near the top
     * sits on a different surface than text near the bottom. So each gradient is
     * *sampled* at the text's real location instead: linear by projecting onto
     * the gradient line, radial by distance from the centre over the
     * farthest-corner radius. A non-gradient layer (`url()`, `none`) is
     * unmeasurable and paints nothing. The radial branch is load-bearing here
     * too: `body` carries two `radial-gradient(circle at …)` washes, one blue at
     * 12% 12% and one green at 88% 6%, and the panels above them are
     * translucent (`--panel` is `rgba(12,20,34,0.84)` in dark and
     * `rgba(255,255,255,0.92)` in light), so both show through.
     */
     ...
     *
     * A driven pass here walks nine panels of prose and — the expensive part —
     * the data tables that are the text alternative to every chart: the SPA
     * operation sequence alone is one row per operation with four cells each,
     * and `#spa-bits` renders three nested spans per exponent bit. All of them
     * re-walk the same ancestors up to `<body>`, through a `.panel` gradient
     * that has to be sampled at each text point. Without the caches the pass
     * re-reads the same computed styles and rects tens of thousands of times.
     ...
     * that is every `.tablewrap` — the `overflow-x: auto` wrapper around each
     * chart's data-table alternative, which exists only once the exhibit that
     * builds it has been run. `.sr-only` is handled separately below.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `checkVisibility()` catches what the `display`/rect tests above miss:
      // `content-visibility: hidden`, and — the case that is live on THIS page —
      // `content-visibility: hidden`. That is live here: every chart's data
      // table lives in a closed `<details class="data">`, and Chromium keeps the
      // last laid-out geometry for a closed disclosure's subtree, so the
      // `display` and rect tests above all pass for rows that paint nothing. The
      // gate this replaces set `.open = true` on all of them from script; this
      // one clicks each `<summary>`, which is the route a reader has, and the
      // guard here is what keeps the shut ones from being measured in between.
      ...
      // `styles/main.css` defines it as the classic recipe. It is unused by
      // this lab's own markup today, which is precisely why the guard has to be
      // measured rather than assumed — the shared top bar's skip link uses the
      // sibling idiom (parked at `top: -3rem`, caught by the document-space test
      // just above), and the first `.sr-only` anyone adds here would otherwise
      // be reported as a contrast failure for ink that is never laid down. The
      // guard is deliberately narrow: only a ZERO-AREA clip qualifies, so a
     ...
     *
     * This lab hides NOTHING with `aria-hidden` in its own markup — checked
     * rather than assumed, and `gate.ts` asserts it in every driven state, so
     * the exclusion cannot quietly start covering something. The only
     * `aria-hidden` elements on the page are the shared top bar's two SVG
     * glyphs. Every chart takes the other route instead: `role="img"` with a
     * written label, which keeps it exposed.
     *
     * That matters here more than usual, because the blind spot is exactly
     * where a live readout could hide: every recovered key byte, rank,
     * correlation and security verdict on this page is real generated output,
     * and an `aria-hidden` anywhere near one would remove it from BOTH oracles
     * at once.
     */
     ...
     *
     * This lab draws no SVG of its own — every figure is a `<canvas>` carrying
     * `role="img"` and a written label, with a `<details class="data">` table as
     * its text alternative, and its only SVG is the shared top bar's two
     * `aria-hidden` glyphs. The guard stays because the usual way the mistake
     * arrives is an array interpolated into a template literal, which JS joins
     * with commas, leaving a run of "," text nodes inside whatever element wraps
     * it — and inside a `<g>` those
     * render nothing while a walk that measured them would report a 1:1 failure
     * describing ink that was never laid down.
     */
     ...
     *
     * It is not a no-op here, and this page takes the FIRST branch: `styles/
     * main.css` sets `html { background-color: var(--bg) }`, so the root's own
     * paint resolves and propagates to the canvas — `#14100a` in dark, `#f4ecdb`
     * in light. `<body>` then adds two `radial-gradient` accent washes on top of
     * the same `--bg`, which the ancestor walk samples at each text point. The
     * document runs nine panels deep, well past the point where an
     * ancestor-only walk stops finding anything opaque.
     */
